...
export class MaintenancePage implements OnInit {
  ...
  updateQuantity(qp: string): void {
    this.horometro = qp;
    const decimals = qp.substr((qp.lastIndexOf('.') + 1), 2);
    console.log(decimals);
    this.horometromask = qp;
  }
  async initscan() {
    this.utils.scan().then(async datascan => {
      ...
      if (response.length > 0) {
        ...
        this.addmain('Tipo de Mantenimiento', 'Preventivo', this.mainobj);
        this.addmain('Servicio', '', this.mainobj);
    ...
    switch ( pas ) {
      ...
      case 3:
        if (this.nombreMantenimiento === '') {
          this.utils.toastPresent('Captura un nombre de servicio');
        ...
        } else if (this.tipoServicio === undefined) {
          this.utils.toastPresent('Selecciona un tipo de servicio');
        } else {
  ...
  private async getTiposServicio() {
    await this.utils.presentLoader();
    // tslint:disable-next-line: max-line-length
    await this.http.get(environment.api + 'DatosEntidad/api/DatosEntidad?_id=Catalogo_de_Servicios&IdCompania=' + this.usuario.IdCompania, {}, {}).then(async data => {
      await this.utils.dismissLoader();
      const response = JSON.parse(data.data);
      console.log(response);
      // tslint:disable-next-line: forin
      for ( const resp in response ) {
        this.tiposServicios.push(response[resp].Campos[0].ValorCampo);
      }
    }).catch(async error => {
      await this.utils.dismissLoader();
      console.log(error);
  ...
  async initpaso3() {
    this.addmain('Servicio', this.tipoServicio, this.mainobj);
    this.addmain('Horas de Servicio / Kilometraje', this.horometro, this.mainobj);
    this.addmain('Realizado Por', this.lugar, this.mainobj);
  ...
  async initpaso4() {
    ...
    // agregar
    await this.utils.presentLoader();
    this.http.post(environment.api + 'DatosEntidad/api/datosentidad', this.consuobj, {}).then(async data => {
      await this.utils.dismissLoader();
      ...
      this.consumibles.push({
        ...
        totalm: '$ ' + this.utils.maskmoney(String((this.cantidad * this.pu))),
        id: idcon
      });
  ...
  async evento(x) {
    await this.utils.presentLoader();
    await this.http.delete(environment.api + 'DatosEntidad/api/datosentidad?_id=' + this.consumibles[x].id, {}, {}).then(async data2 => {
      await this.utils.dismissLoader();
